...
- **v4 (markdown-harness handover):** `GEN-001` is rebased on the `markdown-harness` contract, which splits an ADR's scope into a `files:` check channel and a `paths:` steer channel, adds a 12,000-character size budget, and moves the `.claude/rules` runtime channel out into its own ADR. This repo therefore gains `GEN-002-adr-symlink-claude-rules`, reusing the number the deleted `GEN-002-harness-config` vacated, and **`ADR_CORE` grows to `["GEN-001", "GEN-002"]`**: the CLI writes `.claude/rules` symlinks into every Target (§3, §5), so withholding the rule that checks them would ship an unguarded invariant — and that ADR's §2.3 is inverted against upstream for the same reason, since here the channel is a shipped contract rather than a host-only convenience. Three divergences from upstream are deliberate and marked in-file: `description` is a required frontmatter key, because `.archgate/INDEX.md` carries it per ADR; GEN-002 §2.3; and GEN-001 keeps the [#7](https://github.com/hancrafted/typescript-ai-harness/issues/7)/[#8](https://github.com/hancrafted/typescript-ai-harness/issues/8)/[#9](https://github.com/hancrafted/typescript-ai-harness/issues/9) deferred-hardening links and the toolchain note. Dropped in the rebase: the prose standard (GEN-001 §5.5–5.10) and the `adr-no-review-tag` rule. `GEN-003-frontmatter`, the four supporting files §3 enumerates and `.typescript-ai-harness.json` are now **deleted**, completing the withdrawal v3 deferred — so §2's `ADR_CORE` list, §3's supporting-files bullet, §4's Seeded-config bullet and §6 in their entirety describe a bundle that no longer exists, and `supportingFiles` is `["rules.d.ts"]` alone. The release flow loses its second version stamp with the config that carried it. **Reusing the `GEN-002` number is breaking for Targets:** an install from `v0.1.4` or earlier holds a `GEN-002-harness-config` trio that nothing overwrites, and archgate's loader rejects a duplicate ADR id before any rule runs — `archgate check` exits non-zero with no output, so the Target's whole gate is dead rather than merely noisy. Removal instructions live in `README.md` ("Removing withdrawn ADRs"), to be pointed at from the release notes.
...
- [ADR Contract (GEN-001)](../../.archgate/adrs/GEN-001-adr.md) — forces the bundle's membership (test sibling, size budget, the `files:`/`paths:` split).
- [ADR Symlink for Claude Code Rules (GEN-002)](../../.archgate/adrs/GEN-002-adr-symlink-claude-rules.md) — the `.claude/rules` entry per scoped ADR and the symlink-not-copy mechanic.
