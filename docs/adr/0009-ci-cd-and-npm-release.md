...
The organising rule: **deterministic checks** (source-only, same-input-same-result) run on push/PR/release; the **time-varying security scan** (depends on an external CVE feed) concentrates at the PR gate, release, and nightly, never on a WIP push (see `CONTEXT.md` → CI/CD).
