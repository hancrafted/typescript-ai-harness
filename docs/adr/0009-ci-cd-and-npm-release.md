...
- The **capture-freshness guard** (the ADR-0010 clean-diff guard that fails when a stale Core bundle asset lags its canonical source) runs as its own named step in `verify`, between `archgate` and `build`. The ADR-0010 capture mechanism it depends on (`harness.config.json` + `scripts/capture.ts`, #47) has now landed, so the guard replaces the earlier placeholder: `ci.yml` re-runs `npm run capture` and fails on any drift in `assets/`.
