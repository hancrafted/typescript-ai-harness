...
During active development the tool is run with `npx github:hancrafted/ai-harness-setup`, which clones the repo and executes its `bin`. The `bin` is a small `.mjs` launcher that registers `tsx` (a runtime dependency) and imports the TypeScript entry directly, so there is no compile step and we ship source. We deliberately defer npm publishing + a compiled `dist/` — which would give the clean `npx ai-harness-setup` name and faster cold starts — until the tool stabilises.
...
- **npm publish + compiled `dist/`:** clean `npx ai-harness-setup`, slim/fast installs; costs a build + publish pipeline. A later add-on, not an either/or.
...
- The clean `npx ai-harness-setup` invocation requires a registry publish; until then the invocation is `npx github:hancrafted/…`.
