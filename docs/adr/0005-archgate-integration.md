...
- The Dependency stays **version-ranged** (single `ARCHGATE_VERSION`, surfaced in `harness.config.json` per ADR-0010) so `check`/config behaviour matches the seeded snapshot. "Pinned" overstates it twice: npm's caret fixes only the minor for a `0.x` range, and the installed shim execs whatever binary is already cached at `~/.archgate/bin/archgate` without comparing versions. The range therefore governs a cold install and nothing else — which is exactly where it bites, because `archgate check` regenerates `rules.d.ts` from the running binary and CI always installs cold.
...
- **v5 (superseded, v6):** `rules.d.ts` is committed and seeded as a Core-bundle supporting file (ADR-0010 §3), reversing v1–v4's "deliberately not written, gitignored" stance. Rationale: a freshly-onboarded Target must be able to author its **own** ADR `.rules.ts` with working types/`tsc`/intellisense before its first `archgate check`; the file is `@generated` and regenerates byte-for-byte under the pinned archgate version, so a committed copy stays a clean diff (verified) rather than churning, and no install-time archgate shell-out is reintroduced (v4's reason stands). See ADR-0002 for the mutation-model classification.
- **v6 (current, okf#32 phase 0):** the `.typescript-ai-harness.json` seed leaves the written-files list. It materialised `GEN-003`'s default frontmatter block, and `GEN-003` no longer ships (ADR-0010 v3), so seeding it would hand a Target a config file nothing reads. `.archgate/config.json` and `.claude/settings.local.json` are unaffected and stay Seeded. v5's `rules.d.ts` stance is unchanged — it is the one supporting file that survives the bundle shrink, and the one whose regeneration makes the archgate version range load-bearing.
