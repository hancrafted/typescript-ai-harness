...
// .typescript-ai-harness.json: omit the manifest to model an absent config
...
// ctx.readFile throws. `manifest` is served by readJSON at the config path (and
...
function makeCtx(
  ...
  opts?: { manifest?: unknown; symlinks?: string[]; brokenJson?: boolean },
) {
...
describe('frontmatter-config-valid', () => {
  it('passes the canonical valid configs', async () => {
    for (const m of [VALID_CONFIG, STRICT_CONFIG]) {
      const { ctx, violations } = makeCtx({}, { manifest: m });
  ...
  it('no-ops when the config file, namespace, or block is absent', async () => {
    for (const opts of [undefined, { manifest: { markdown: { version: 1 } } }, { manifest: RETIRED_KEY_CONFIG }]) {
  ...
  it('fails non-kebab allowedTypes', async () => {
    const m = manifest([{ include: ['x.md'], rule: { allowedTypes: ['Design ADR'] } }]);
    const { ctx, violations } = makeCtx({}, { manifest: m });
    await configValid.check(ctx);
  ...
  it('fails an invalid label pin', async () => {
    const m = manifest([{ include: ['x.md'], rule: { label: 'heading' } }]);
    const { ctx, violations } = makeCtx({}, { manifest: m });
    await configValid.check(ctx);
  ...
  it('fails a non-boolean requireDescription', async () => {
    const m = manifest([{ include: ['x.md'], rule: { requireDescription: 'yes' } }]);
    const { ctx, violations } = makeCtx({}, { manifest: m });
    await configValid.check(ctx);
  ...
  it('fails non-positive-integer caps', async () => {
    for (const rule of [{ maxLabel: 12.5 }, { maxDescription: -3 }, { maxTag: 0 }]) {
      const { ctx, violations } = makeCtx({}, { manifest: manifest([{ include: ['x.md'], rule }]) });
      await configValid.check(ctx);
  ...
  it('fails an unknown settings key', async () => {
    const m = manifest([{ include: ['x.md'] }], { settings: { draftEscap: true } });
    const { ctx, violations } = makeCtx({}, { manifest: m });
    await configValid.check(ctx);
  ...
  it('fails a non-boolean draftEscape', async () => {
    const m = manifest([{ include: ['x.md'] }], { settings: { draftEscape: 'yes' } });
    const { ctx, violations } = makeCtx({}, { manifest: m });
    await configValid.check(ctx);
...
describe('frontmatter-floor', () => {
  const adrRules = manifest([{ include: ['docs/adr/*.md'], rule: { allowedTypes: ['design-adr'], label: 'title' } }]);
  ...
  it('accepts type: draft when settings.draftEscape is on', async () => {
    const dm = manifest([{ include: ['docs/adr/*.md'], rule: { allowedTypes: ['design-adr'], label: 'title' } }], {
  ...
  it('accepts any kebab type in a membership-open entry (rule without allowedTypes)', async () => {
    const openRules = manifest([{ include: ['docs/adr/*.md'], rule: { label: 'title' } }]);
  ...
  it('in an open governed entry (no rule payload), fails when neither name nor title is present', async () => {
    const openRules = manifest([{ include: ['docs/adr/*.md'] }]);
    const files = { 'docs/adr/a.md': md('type: design-adr\ndescription: "Why."') };
    const { ctx, violations } = makeCtx(files, { manifest: openRules });
    await floor.check(ctx);
  ...
  it('an excluded file matching no later entry falls to unmatched (exempt by default)', async () => {
    const m = manifest([{ include: ['docs/**/*.md'], exclude: ['docs/notes.md'], rule: { label: 'title' } }]);
...
describe("frontmatter-floor coverage (unmatched: 'error')", () => {
  it('errors on an in-coverage file that no entry claims, and only on it', async () => {
    ...
    const { ctx, violations } = makeCtx(files, { manifest: STRICT_CONFIG });
    await floor.check(ctx);
...
describe('frontmatter-floor consumer contract (all-or-nothing)', () => {
  ...
  it('governs nothing on a spine-invalid block', async () => {
    const { ctx, violations } = makeCtx(failingFiles, { manifest: SPINE_INVALID_CONFIG });
    await floor.check(ctx);
    expect(violations).toEqual([]);
  });

  it('governs nothing on a payload-invalid block', async () => {
    const { ctx, violations } = makeCtx(failingFiles, { manifest: PAYLOAD_TYPO_CONFIG });
    await floor.check(ctx);
    expect(violations).toEqual([]);
  });

  it('governs nothing on a format-version skew', async () => {
    const { ctx, violations } = makeCtx(failingFiles, { manifest: VERSION_MISMATCH_CONFIG });
    await floor.check(ctx);
    expect(violations).toEqual([]);
  });
  ...
  it('falls back to the built-in default when only retired namespaces are present', async () => {
    ...
    const { ctx, violations } = makeCtx(failingFiles, { manifest: RETIRED_KEY_CONFIG });
    await floor.check(ctx);
  ...
  it('falls back to the built-in default when the block name is a typo', async () => {
    const { ctx, violations } = makeCtx(failingFiles, { manifest: UNREGISTERED_BLOCK_CONFIG });
    await floor.check(ctx);
    expect(violations.some((v) => v.file === 'README.md')).toBe(true);
  });
  ...
  it('governs by the shared VALID_CONFIG fixture end to end', async () => {
    ...
    const { ctx, violations, warnings } = makeCtx(files, { manifest: VALID_CONFIG });
    await floor.check(ctx);
    expect(violations).toEqual([]);
...
describe('frontmatter-floor built-in default (no harness config)', () => {
  ...
  it('applies the default when a healthy config omits the frontmatter block', async () => {
    ...
    const { ctx, violations } = makeCtx(files, { manifest: { markdown: { version: 1 } } });
    await floor.check(ctx);
    expect(violations.some((v) => v.file === 'README.md')).toBe(true);
  });
